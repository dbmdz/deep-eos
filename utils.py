"""utils: Helper methods for deep-eos."""
...
class Utils():
    """utils: implement various helper methods for deep-eos."""

    def build_char_2_id_dict(self, data_set_char, min_freq):  # pylint: disable-msg=no-self-use # noqa: E501
        """Build a char_to_id dictionary.
        ...
        # Arguments
          ...
          min_freq     : Defines the minimum frequency a char must appear in
                         data set
        ...
        for k, v in [(k, v) for k, v in char_freq.items() if v >= min_freq]:  # pylint: disable-msg=invalid-name # noqa: E501
            char_2_id_table[k] = id_counter
            id_counter += 1

        return char_2_id_table

    def build_data_set(self, data_set_char, char_2_id_dict, window_size): # pylint: disable-msg=no-self-use # noqa: E501
        """Build a "real" data set with numpy compatible feature vectors.
    ...
    def build_data_set_char(self, t, window_size): # pylint: disable-msg=no-self-use # noqa: E501
        """Build a data set from corpus.
    ...
    def build_potential_eos_list(self, t, window_size): # pylint: disable-msg=no-self-use # noqa: E501
        """Build a list of potential eos from a given text.
        ...
        # Returns
          ...
          potential eos. The second position holds the extracted character
          sequence.
        """
        punct = r'[\(\)\u0093\u0094`“”\"›〈⟨〈<‹»«‘’–\'``'']*'
        ...
        for eos_position in eos_positions:
            ...
            cleaned_left_context = re.sub(r'\s+', ' ', cleaned_left_context)
            cleaned_right_context = re.sub(r'\s+', ' ', cleaned_right_context)

            potential_eos_position.append((eos_position, cleaned_left_context[-window_size:] + t[eos_position] + cleaned_right_context[1:window_size + 1])) # noqa: E501

        return potential_eos_position

    def save_vocab(self, char_2_id_dict, vocab_filename): # pylint: disable-msg=no-self-use # noqa: E501
        ...
        """
        with open(vocab_filename, 'wb') as f_p:
            pickle.dump(char_2_id_dict, f_p, pickle.HIGHEST_PROTOCOL)

    def load_vocab(self, vocab_filename): # pylint: disable-msg=no-self-use # noqa: E501
        ...
        """
        with open(vocab_filename, 'rb') as f_p:
